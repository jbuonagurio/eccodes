...
typedef size_t   (*readproc)(reader*,void*,size_t,int*);
typedef int   (*seekproc)(reader*,off_t);
typedef off_t   (*tellproc)(reader*);
typedef void* (*allocproc)(reader*,size_t*,int*);
...
static int read_the_rest(reader* r,size_t message_length,unsigned char* tmp, int already_read, int check7777)
{
    ...
    buffer = (unsigned char*)r->alloc(r,&buffer_size,&err);
    if(err) return err;
    ...
    if((r->read(r,buffer+already_read,rest,&err) != rest) || err)
        return err;
...
static int read_GRIB(reader* r)
{
    ...
    case 1:
        if (r->headers_only) {
            ...
            if((r->read(r,tmp+i,sec1len-3,&err) != sec1len-3) || err)
                return err;
            flags = tmp[15];
            ...
            if(flags & (1<<7)) {
                ...
                i+=3;
                /* Read section 2 */
                if((r->read(r,tmp+i,sec2len-3,&err) != sec2len-3) || err)
                    return err;
                i += sec2len-3;
            }
            ...
            if(flags & (1<<6)) {
                ...
                if((r->read(r,tmp+i,sec3len-3,&err) != sec3len-3) || err)
                    return err;
                i += sec3len-3;
            }
            ...
            length=i;
            err=r->seek(r,total_length-length-1);
        ...
        else if(length & 0x800000)
        {
            ...
            /* table version */
            if(r->read(r,&tmp[i++],1,&err) != 1 || err) return err;
            /* center */
            if(r->read(r,&tmp[i++],1,&err) != 1 || err) return err;
            /* process */
            if(r->read(r,&tmp[i++],1,&err) != 1 || err) return err;
            ...
            if((r->read(r,tmp+i,sec1len-3-5,&err) != sec1len-3-5) || err)
                return err;
    ...
    default:
        r->seek_from_start(r,r->offset+4);
...
static int read_HDF5_offset(reader *r, int length, unsigned long* v, unsigned char *tmp, int* i) {
    ...
    if( (r->read(r, buf, length, &err) != length) || err) {
        return err;
    }
...
static int read_HDF5(reader *r)
{
    ...
    if( (r->read(r, &version_of_superblock, 1,  &err)  != 1) || err) {
        return err;
    }
    ...
    if (version_of_superblock == 2 || version_of_superblock == 3) {
        if( (r->read(r, &size_of_offsets, 1, &err) != 1) || err) {
            return err;
        }
        ...
        if( (r->read(r, &size_of_lengths, 1, &err) != 1) || err) {
            return err;
        }

        tmp[i++] = size_of_lengths;

        if( (r->read(r, &consistency_flags, 1, &err) != 1) || err) {
            return err;
        }
        ...
        if( (r->read(r, &version_of_file_free_space, 1, &err) != 1) || err) return err;
        tmp[i++] = version_of_file_free_space;

        if( (r->read(r, &version_of_root_group_symbol_table, 1, &err) != 1) || err) return err;
        tmp[i++] = version_of_root_group_symbol_table;

        if( (r->read(r, &ch, 1, &err) != 1) || err) return err; /* reserved */
        tmp[i++] = ch;

        if( (r->read(r, &version_number_shared_header, 1, &err) != 1) || err) return err;
        tmp[i++] = version_number_shared_header;

        if( (r->read(r, &size_of_offsets, 1, &err) != 1) || err) return err;
        tmp[i++] = size_of_offsets;
        if (size_of_offsets > 8) {
            grib_context_log(c, GRIB_LOG_ERROR,"read_HDF5: invalid size_of_offsets: %ld, only <= 8 is supported", (long)size_of_offsets);
            return GRIB_NOT_IMPLEMENTED;
        ...
        if( (r->read(r, &size_of_lengths, 1, &err) != 1) || err) return err;
        tmp[i++] = size_of_lengths;

        if( (r->read(r, &ch, 1, &err) != 1) || err) return err; /*reserved*/
        tmp[i++] = ch;

        if( (r->read(r, &skip, 4, &err) != 4) || err) return err; /* Group Leaf/Internal Node K: 4 bytes */
        tmp[i++] = skip[0]; tmp[i++] = skip[1]; tmp[i++] = skip[2]; tmp[i++] = skip[3];

        if( (r->read(r, &skip, 4, &err) != 4) || err) return err; /* consistency_flags: 4 bytes */
        tmp[i++] = skip[0]; tmp[i++] = skip[1]; tmp[i++] = skip[2]; tmp[i++] = skip[3];

        if (version_of_superblock == 1) {
            /* Indexed storage internal node K and reserved: only in version 1 of superblock */
            if( (r->read(r, &skip, 4, &err) != 4) || err) return err;
            tmp[i++] = skip[0]; tmp[i++] = skip[1]; tmp[i++] = skip[2]; tmp[i++] = skip[3];
        }
...
static int _read_any(reader *r, int grib_ok, int bufr_ok, int hdf5_ok, int wrap_ok)
{
    ...
    while(r->read(r,&c,1,&err) == 1 && err == 0)
    {
        magic <<= 8;
        magic |= c;
    ...
    while(r->read(r,&c,1,&err) == 1 && err == 0)
    {
        ...
        if (magic == start) {
            ...
            while(r->read(r,&c,1,&err) == 1 && err == 0) {
                ...
                magic &= 0xffffffff;
                if (magic == theEnd) {
                    r->seek(r,already_read-message_size);
                    buffer = (unsigned char*)r->alloc(r,&message_size,&err);
                    if (!buffer) return GRIB_OUT_OF_MEMORY;
                    if (err) return err;
                    memcpy(buffer,tmp,already_read);
                    r->read(r,buffer+already_read,message_size-already_read,&err);
...
off_t stdio_tell(reader *r)
{
    return grib_context_tell(r->context, r->read_data);
}

int stdio_seek(reader *r, off_t offset)
{
    int err = 0;
    if (grib_context_seek(r->context, offset, SEEK_CUR, r->read_data)) err=GRIB_IO_PROBLEM;
    return err;
}

int stdio_seek_from_start(reader *r, off_t offset)
{
    int err = 0;
    if (grib_context_seek(r->context, offset, SEEK_SET, r->read_data)) err=GRIB_IO_PROBLEM;
    return err;
}

size_t stdio_read(reader *r, void *buffer, size_t len, int *err)
{
    size_t n;

    if (len==0) return 0;

    n = grib_context_read(r->context, buffer, len, r->read_data);
...
static off_t stream_tell(reader* r)
{
    return 0;
}

static int stream_seek(reader* r,off_t len)
{
    return 0;
}
static size_t stream_read(reader* r,void* buffer,size_t len,int* err)
{
    stream_struct *s = (stream_struct*)r->read_data;
    long n = len;
...
static off_t memory_tell(reader* r)
{
    return 0;
}

static int memory_seek(reader* r,off_t len)
{
    return 0;
}

static size_t memory_read(reader* r,void* buf,size_t len,int* err)
{
    memory_read_data *m = (memory_read_data*)r->read_data;
